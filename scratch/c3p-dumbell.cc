...
*/
NS_LOG_COMPONENT_DEFINE ("C3pDumbell");
...
SetupTopo (uint32_t srcNum, uint32_t dstNum, DataRate edgeBW, DataRate switchBW, Time linkDelay)
{
  ...
  for (NodeContainer::Iterator i = srcs.Begin (); i != srcs.End (); ++i)
    {
      auto interfaces = MakeLink ("pfifo", *i, routers.Get (0), edgeBW, linkDelay, ipv4AddrHelper);
      src_interfaces.Add (interfaces.Get (1));
    }

  MakeLink ("red", routers.Get (0), routers.Get(1), switchBW, linkDelay, ipv4AddrHelper);

  for (NodeContainer::Iterator i = dsts.Begin (); i != dsts.End (); ++i)
    {
      auto interfaces = MakeLink ("pfifo", routers.Get (1), *i, edgeBW, linkDelay, ipv4AddrHelper);
