...
NS_LOG_COMPONENT_DEFINE ("SCCGoodputTest");
...
// c3 attributes
Time tunnel_interval ("6ms");
Time division_interval ("3ms");
DataRate tunnel_bw ("100Mbps"); //!< initial tunnel bw
...
SetConfig (bool useEcn, bool useDctcp)
{
  ...
  Config::SetDefault ("ns3::dcn::C3Division::RateThresh", DataRateValue (DataRate(link_data_rate)));
  ...
  Config::SetDefault ("ns3::dcn::C3Tunnel::IsExperiment", BooleanValue (true));
}
...
main (int argc, char *argv[])
{
  ...
  dctcpHelper.Install (clients);
  dctcpHelper.Install (servers);
