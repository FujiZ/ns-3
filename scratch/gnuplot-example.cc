...
{
  std::string fileNameWithNoExtension = "plot-2d";//没有扩展名的文件名
  std::string graphicsFileName        = fileNameWithNoExtension + ".png";//图片文件名
  std::string plotFileName            = fileNameWithNoExtension + ".plt";//Gnuplot控制文件名
  std::string plotTitle               = "2-D Plot";//图片标题
  std::string dataTitle               = "2-D Data";//数据标题
  ...
  plot.SetTerminal ("png");//设置图片格式

  // Set the labels for each axis.
  plot.SetLegend ("X Values", "Y Values");//设置坐标标签
  ...
  dataset.SetStyle (Gnuplot2dDataset::LINES_POINTS);//线型
  ...
  for (x = -5.0; x <= +5.0; x += 1.0)
    {
      ...
      // Add this point.
      dataset.Add (x, y);//向对象数据添加坐标点
    }

  // Add the dataset to the plot.
  plot.AddDataset (dataset);//把数据对象添加到gnuplot对象
...
void Create2DPlotWithErrorBarsFile ()
{
  ...
  Gnuplot2dDataset dataset;
  dataset.SetTitle (dataTitle);
  dataset.SetStyle (Gnuplot2dDataset::POINTS);//点型
